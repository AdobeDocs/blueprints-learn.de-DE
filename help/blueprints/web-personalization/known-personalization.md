...
* Adobe Analytics oder Customer Journey Analytics (optional): Fügt die Möglichkeit hinzu, Segmente basierend auf historischen Kunden- und Verhaltensdaten mit einer feinkörnigen Segmentierung zu erstellen
...
* Zur Verwendung im Mobile SDK ist die [Adobe Journey Optimizer - Decisioning-Erweiterung](https://aep-sdks.gitbook.io/docs/using-mobile-extensions/adobe-journey-optimizer-decisioning) muss im Mobile SDK installiert sein.
...
1. Implementierung [Experience Platform Web SDK](https://experienceleague.adobe.com/docs/experience-platform/edge/home.html?lang=de) oder [Experience Platform Mobile SDK](https://aep-sdks.gitbook.io/docs/) mit der richtigen Erweiterung (Target oder Adobe Journey Optimizer - Decisioning) installiert. Experience Platform Web/Mobile SDK oder EDGE API sind für die Echtzeit-Edge-Segmentierung erforderlich, aber nicht für die Freigabe von Streaming- und Batch-Zielgruppen von Real-time Customer Data Platform nach Target.
...
* Beachten Sie, dass Daten, die vor dem Besuch oder der Anmeldung bei einer Website durch einen Verbraucher in den Hub hochgeladen wurden, nicht sofort zur Personalisierung verfügbar sind. Zunächst muss ein aktives Kantenprofil vorhanden sein, damit Hub-Daten synchronisiert werden können. Nach der Erstellung synchronisiert sich das Edge-Profil asynchron mit dem Hub-Profil, was zu einer nächsten Seitenpersonalisierung führt.
* Die Freigabe von Zielgruppen von Adobe Experience Platform für Adobe Target erfordert die Verwendung von ECID als Identität bei der Verwendung des Zielgruppenfreigabe-Service, wie oben in Integrationsparameter 2 und 3 beschrieben.
* Alternative Identitäten können auch verwendet werden, um Experience Platform-Zielgruppen über Audience Manager für Adobe Target freizugeben. Experience Platform aktiviert Zielgruppen für Audience Manager über die folgenden unterstützten Namespaces: IDFA, GAID, AdCloud, Google, ECID, EMAIL_LC_SHA256. Beachten Sie, dass Audience Manager und Target Zielgruppenmitgliedschaften über die ECID-Identität auflösen. Daher muss sich ECID weiterhin im Identitätsdiagramm befinden, damit der Verbraucher die endgültige Zielgruppenfreigabe an Adobe Target vornehmen kann.
